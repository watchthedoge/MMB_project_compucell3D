...
class GrowthSteppableLinear(SteppableBasePy):
    ...
    def step(self,mcs):
        ...
        tvadded=0
        totV=0
        EQUAL_GROWTH_END = relaxtime 

        if mcs < EQUAL_GROWTH_END:
            gr_wt = growthratescrb  # force equal growth
            gr_scrb = growthratescrb
        else:
            gr_wt = growthratewt
            gr_scrb = growthratescrb
        if mcs>relaxtime:
            ...
            for cell in self.cellList:
                ...
                if cell.type==1 or cell.type==3 or cell.type==5 or cell.type==7:
                    delta = round(random.normalvariate(gr_wt, 2.5))
                    delta = max(0, delta)
                    ...
                    delta = round(random.normalvariate(gr_scrb, 2.5))
                    delta = max(0, delta)
                    cell.targetVolume += delta * np.exp(-(CI/mcs)*((cell.volume-cell.targetVolume)**2))
...
class cleanup(SteppableBasePy):
    ...
    def step(self,mcs):
        ...
        for cell in self.cellList:
            if cell.type == 9 or cell.type == 10:
                apop_mcs = cell.dict.get("apt", None)
                if apop_mcs is not None and (mcs - apop_mcs) >= 5:
